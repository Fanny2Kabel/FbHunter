...
|__] |__/ | |  | |__|  |  |___ |___ |__] 
\033[0m|    |  \ |  \/  |  |  |  |___ |    |__]  {p}
{H}INFO AUTHOR : {M}FANNY 2 KABEL{p}\n{M}NOTE! {P}: {H}Sederhana Tapi Sangat Mengerikan''')
# * [ BAGIAN COOLI ] * #
def gafi_login():
	try:
		os.system('clear')
...
# * * [ OWH JELAS DONK AINK KAN COWOK ] * * #
		ses = requests.Session()
		__fanny_kabel___(f'{H} Masukan Cookies')
		cookie=input(f' {M} Cookies :{H} ')
		with requests.Session() as r:
			try:
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					print(f"{H}Cookie Invalid...", end='\r');time.sleep(3.5);print("                     ", end='\r');exit()
				else:
...
def menu(name):
	try:
		...
		__fanny_kabel___(f'{H} Cookie Exp...')
		time.sleep(3)
		gafi_login()
	os.system('clear')
	___fanny_ganteng___()
		...
		with tred(max_workers=30) as fannykabel:
			for kocak_euy in id2:
				...
				if 'async' in fannyxxx:
					...
					fannykabel.submit(_async_2_,idf,afa_aja_boleh)
		print('')
		print(f'{H} Hasil : %s\n{K} Hasil : %s'%(fanny_ok,fanny_cp))
		print('')
		print(f'{H} FANNY_OK : {H}%s '%(ok))
		print(f'{K} FANNY_CP : {K}%s '%(cp))
		print('')
# * [ METHODE NGOCOK ] * #
